/**
 * @file src/pages/profile/[username]/followers.tsx
 * @description Followers list page (Reddit-ish: simple header, card list, good empty/error, no blank screen)
 */
...
export default function FollowersPage() {
  ...
  useEffect(() => {
    ...
    const fetchFollowers = async () => {
      try {
        ...
        const res = await followAPI.getFollowers(username, page);
        const data: User[] = res.data?.data?.followers || res.data?.data || [];

        setFollowers((prev) => (page === 1 ? data : [...prev, ...data]));
  ...
  const filtered = useMemo(() => {
    ...
    return followers.filter((u) => u.username.toLowerCase().includes(term));
  }, [followers, q]);
  ...
  return (
    <>
      <Head>
        <title>{username}&apos;s Followers | DevMatch</title>
        <meta
          name="description"
          content={`People following @${username} on DevMatch`}
        />
      </Head>
          ...
          {!error && followers.length === 0 && (
            <div className="rounded-2xl border border-gray-200 bg-white p-10 text-center shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-white/50">
                This is normal for new profiles. If you expect followers here,
                your API mapping is wrong.
              </p>
            </div>
          )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <p className="truncate text-sm font-semibold text-gray-900 group-hover:text-gray-900 dark:text-white">
                              @{u.username}
                            </p>
